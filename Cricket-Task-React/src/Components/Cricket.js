...
import { stateA, stateB, runsRateA, runsRateB } from '../Actions/Score';
...
function Cricket() {
    ...
    const TeamAFunc = () => {
        ...
        for (i; i <= over; i++) {
            ...
            arr.push(
                <>
                    <h1> Over No : {i} </h1>
                    <TeamA over={over} state={statea} change={setValueA} wicket={wicketA} ball={ballsA} runrate={runRateA} />
                </>
            )
        }
    ...
    const TeamBFunc = () => {
        ...
        for (i; i <= over; i++) {
            ...
            arr.push(
                <>
                    <h1> Over No : {i} </h1>
                    <TeamB over={over} state={stateb} change={setValueB} wicket={wicketB} ball={ballsB} runrate={runRateB} />
                </>
            )
        }
    ...
    const Result = () => {
        ...
        if ((ballsA === over * 6 || wicketA === 10) && (scoreB > scoreA || wicketB === 10 || ballsB === over * 6)) {
