...
import { incrementA, stateA, wicketA, ballA } from '../Actions/Score';
...
function TeamA(props) {
    ...
    let playerChecker = (player) => {
        if (run > 0) {
            ...
            if (state[player].status) {
                ...
                dispatch(incrementA(run));  // Incrementing score
                dispatch(ballA());    // Incrementing ball count
            ...
            if (state[player].status) {
                ...
                dispatch(ballA());    // Incrementing ball count
                dispatch(wicketA());  // Incrementing wicket count
    ...
    const keyPress = (e) => {
        if (e.keyCode === 13 && ballsUsed < (over * 6) && wicket < 10)
            updateVal();   // Upadating state 
    }
