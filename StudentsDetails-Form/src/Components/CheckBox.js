...
class CheckBox extends React.Component {
    ...
    state = {
        ...
        }, Status: { Available: false, UnAvailable: false }
    };
    ...
    render() {
        ...
        if (this.props.status) {
            ...
            if (this.state.Status.Available === true)
                this.setState({ Status: { Available: false } });

            if (this.state.Status.UnAvailable === true)
                this.setState({ Status: { UnAvailable: false } });
        ...
        let displayStatus = this.props.flexibility;
        displayStatus = Object.keys(this.state.Status).filter((x) => this.state.Status[x]);
        this.props.statusValue(displayStatus + "");

        return (<>
            {/* / {(this.props.status === true) ? this.setState({ sub: { Physics: false, Chemistry: false, Maths: false } }) : ""} */}
            ...
            <input type='checkbox' value={this.state.clas.twelve} name='twelve' onChange={this.checkClass} checked={this.state.clas.tweleve} /> <label> 12 </label> <br />
            ...
            Status:
            <input type='radio' name='Available' value={this.state.Status.Available}  onChange={this.checkStatus} /> <label for={this.state.Status.Available} >Available </label>
            <input type='radio' name='Unavailable' value={this.state.Status.UnAvailable} checked={this.state.Status.UnAvailable} onChange={this.checkStatus} /> <label>Un-Available </label> <br />

            {/* {this.props.status ? {unCheck} : ""} */}
